...
class NatNetClient:
    ...
    def __createDataSocket( self, port ):
        ...
        result.bind( ('', port) )  # '' means all availabel interfaces

        mreq = struct.pack("4sl", socket.inet_aton(self.multicastAddress), socket.INADDR_ANY)  # specify interface
        result.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq) # membership request
		
		'''
		IPPROTO_IP is a socket option
		
		IP_ADD_MEMBERSHIP is used to Join the socket to the supplied multicast group on the specified interface
		
		mreq is the specified interface which contains the multicast IP addresss and the number of available 
		addresses, all in bytes format.
		
		socket.INADDR_ANY returns an integer
		
		socket.inet_aton takes the numbers-and-points IP address and returns a byte which contains 4 strings.
		So, to keep it as it is we need to use 4s while packing it. The l is used for the INADDR_ANY's returned value
		
		References: 
		[1] https://msdn.microsoft.com/en-us/library/windows/desktop/ms738586(v=vs.85).aspx
		[2] http://www.delorie.com/gnu/docs/glibc/libc_352.html
		[3] https://linux.die.net/man/3/inet_aton
		''' 
    ...
    def __createCommandSocket( self ):	
        ...
        result.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        result.bind( ('', 0) )  # '' means all availabel interfaces
        result.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)
		'''
		SO_BROADCAST This option controls whether datagrams may be broadcast from the socket.
    ...
    def __processMessage( self, data ):
        ...
        offset = 4  # skip the first 4 bytes (they contain Message ID and Packet Size)
        # check white kind of message are we receiving (DataDescription, ModelDef, etc.)
		if( messageID == self.NAT_FRAMEOFDATA ):
            self.__unpackMocapData( data[offset:] )
        
		elif( messageID == self.NAT_MODELDEF ):  # contains description of Mocap Data (FRAMEOFDATA)
            ...
            self.__natNetStreamVersion = struct.unpack( 'BBBB', data[offset:offset+4] )  # can be changes to 4B
            offset += 4
    ...
    def sendCommand( self, command, commandStr, socket, address ):
        ...
        data = command.to_bytes( 2, byteorder='little' )  # command is an integer and it has a to_bytes ethod
        data += packetSize.to_bytes( 2, byteorder='little' )
        
        data += commandStr.encode( 'utf-8' )  # default is already 'utf-8'
        data += b'\0'
		...
		- packetsize - 2 bytes
		- commandStr
		- b'\0'  -> marks the end of the message
